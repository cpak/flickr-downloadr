const { string, int, values, sanitize: _sanitize, sanitizePartial: _sanitizePartial } = require('./repo-utils')
...
const INSERT_SQL = tableName => `INSERT INTO ${tableName}
  (id, type, date_taken, bytes, url, owner, secret, server, farm, title, path)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
...
const GET_BY_TYPE_SQL = tableName => `SELECT * FROM ${tableName} WHERE type = ?`
const getByType = (client, tableName) => type => client.all(GET_BY_TYPE_SQL(tableName), [string(type)])
...
const createRepo = (client, tableName = DEFAULT_TABLE_NAME) => ({
  ...
  getByType: getByType(client, tableName),
