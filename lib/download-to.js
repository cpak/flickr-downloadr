...
const debug = _debug('downloadr:download')
const debugProgress = _debug('downloadr:download:progress')
...
const downloadTo = dirPath => ({ url }) => new Promise((resolve, reject) => {
  ...
  const dl = () => {
    ...
    data.on('downloadProgress', throttle(({ percent, total }) => {
      ...
      debugProgress(`${url} ${Math.round(percent * 100)}%`)
    }), 5000, true)
    ...
    const onFileWritten = ifElseP(checkSize, logAnd(`Downloaded ${url} to ${filePath}`, done(filePath)), logAnd(`Retrying ${url}`, dl))
