...
const downloadTo = (api, dirPath) => ({ url }) => new Promise((resolve, reject) => {
  ...
  const dl = () => api
    ...
    .then(({ status, headers, data }) => {
      ...
      const onFileWritten = ifElseP(checkSize, logAnd(`Downloaded ${url} to ${filePath}`, done(expectedBytes)), logAnd(`Retrying ${url}`, dl))
